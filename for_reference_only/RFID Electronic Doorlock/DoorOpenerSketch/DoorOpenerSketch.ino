...
void ActivateRelais(byte u8_Flags)
{
    ...
    {
        Utils::DelayMilli(500); // make a pause between activation of the relais
    }
...
// This function measures how much the battery voltage decreases when the door opener solenoids are drawing their current (approx 1 to 2 Ampere).
// returns true if the battery is sane (the voltage stays constant).
// The older the battery gets the higher becomes its impedance and the voltage decreases when the door is opened.
// This must be detected and displayed to the user by flashing the green and red LED alternatingly.
// In this case the battery must be replaced soon.
bool ActivateRelaisCheckVoltage(byte u8_Pin)
{
    ...
    uint32_t u32_VoltLow = MeasureVoltage();
    Utils::WritePin(u8_Pin, LOW); // Relais off

    if (u32_VoltLow > u32_VoltHigh)
        return true; // this may happen if the voltage stays constant and there is some noise present.
